defmodule Boldsign.ContactGroupTest do
  ...
  test "list/2 sends GET request", %{bypass: bypass, client: client} do
    Bypass.expect(bypass, "GET", "/v1/contactGroups/list", fn conn ->
  ...
  test "get/2 sends GET request", %{bypass: bypass, client: client} do
    Bypass.expect(bypass, "GET", "/v1/contactGroups/get", fn conn ->
      conn = fetch_query(conn)
  ...
  test "create/2 sends POST request", %{bypass: bypass, client: client} do
    Bypass.expect(bypass, "POST", "/v1/contactGroups/create", fn conn ->
  ...
  test "update/3 sends PUT request", %{bypass: bypass, client: client} do
    Bypass.expect(bypass, "PUT", "/v1/contactGroups/update", fn conn ->
      conn = fetch_query(conn)
      {params, conn} = read_json_body(conn)
  ...
  test "delete/2 sends DELETE request", %{bypass: bypass, client: client} do
    Bypass.expect(bypass, "DELETE", "/v1/contactGroups/delete", fn conn ->
      conn = fetch_query(conn)
      assert conn.query_params["groupId"] == "group_123"
      send_resp(conn, 204, "")
    end)

defmodule Boldsign.ContactTest do
  ...
  test "list/2 sends GET request", %{server: server, client: client} do
    Boldsign.TestHTTPServer.expect(server, "GET", "/v1/contacts/list", fn conn ->
  ...
  test "get/2 sends GET request", %{server: server, client: client} do
    Boldsign.TestHTTPServer.expect(server, "GET", "/v1/contacts/get", fn conn ->
      conn = fetch_query(conn)
  ...
  test "create/2 sends POST request", %{server: server, client: client} do
    Boldsign.TestHTTPServer.expect(server, "POST", "/v1/contacts/create", fn conn ->
  ...
  test "update/3 sends PUT request", %{server: server, client: client} do
    Boldsign.TestHTTPServer.expect(server, "PUT", "/v1/contacts/update", fn conn ->
      conn = fetch_query(conn)
      {params, conn} = read_json_body(conn)
  ...
  test "delete/2 sends DELETE request", %{server: server, client: client} do
    Boldsign.TestHTTPServer.expect(server, "DELETE", "/v1/contacts/delete", fn conn ->
      conn = fetch_query(conn)
      assert conn.query_params["id"] == "contact_123"
      send_resp(conn, 204, "")
    end)

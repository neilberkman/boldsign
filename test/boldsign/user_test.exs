defmodule Boldsign.UserTest do
  ...
  test "list/2 sends GET request", %{bypass: bypass, client: client} do
    Bypass.expect(bypass, "GET", "/v1/users/list", fn conn ->
  ...
  test "get/2 sends GET request", %{bypass: bypass, client: client} do
    Bypass.expect(bypass, "GET", "/v1/users/get", fn conn ->
      conn = fetch_query(conn)
  ...
  test "create/2 sends POST request", %{bypass: bypass, client: client} do
    Bypass.expect(bypass, "POST", "/v1/users/create", fn conn ->
  ...
  test "update/2 uses PUT", %{bypass: bypass, client: client} do
    Bypass.expect(bypass, "PUT", "/v1/users/update", fn conn ->
      {params, conn} = read_json_body(conn)
  ...
  test "change_team/2 extracts userId from params", %{bypass: bypass, client: client} do
    Bypass.expect(bypass, "PUT", "/v1/users/changeTeam", fn conn ->
      conn = fetch_query(conn)
      {params, conn} = read_json_body(conn)
  ...
  test "update_metadata/2 uses PUT", %{bypass: bypass, client: client} do
    Bypass.expect(bypass, "PUT", "/v1/users/updateMetaData", fn conn ->
      {params, conn} = read_json_body(conn)
      assert params["userId"] == "user_123"
  ...
  test "resend_invitation/2 uses UserId query casing", %{bypass: bypass, client: client} do
    Bypass.expect(bypass, "POST", "/v1/users/resendInvitation", fn conn ->
      conn = fetch_query(conn)
      assert conn.query_params["UserId"] == "user_123"
      send_resp(conn, 200, "")
    end)
  ...
  test "cancel_invitation/2 uses UserId query casing", %{bypass: bypass, client: client} do
    Bypass.expect(bypass, "POST", "/v1/users/cancelInvitation", fn conn ->
      conn = fetch_query(conn)
      assert conn.query_params["UserId"] == "user_123"
      send_resp(conn, 200, "")
    end)
